  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: compact ? 6 : 8, width: "100%" }}>
      {players.slice(0, maxRows).map((p, i) => {
        ...
        const borderColor = p.rank === 1 ? "rgba(246,207,125,0.45)"
          : p.rank === 2 ? "rgba(114,240,255,0.22)"
          : p.rank === 3 ? "rgba(255,134,194,0.18)"
          : "rgba(148,163,184,0.14)";
        const bgColor = p.rank === 1 ? "rgba(246,207,125,0.08)"
          : isTop3 ? "rgba(114,240,255,0.05)"
          : "rgba(15,25,43,0.72)";
          ...
          <div key={p.id} className={`animate-rank-in ${p.rank === 1 ? "leaderboard-glow" : ""}`}
            ...
            <div style={{
              ...
              <div style={{
                ...
                color: p.rank === 1 ? "var(--amber)" : p.rank === 2 ? "var(--cyan)" : p.rank === 3 ? "var(--pink)" : "var(--muted)",
              ...
              {showChange && p.lastPoints > 0 && (
                <div style={{
                  ...
                  color: "var(--green)", flexShrink: 0,
                    textShadow: "0 0 8px rgba(127,231,198,0.5)",
                }}>
              ...
              <div style={{
                ...
                fontSize: compact ? "0.82rem" : "0.92rem",
                color: p.rank === 1 ? "var(--amber)" : isTop3 ? "var(--cyan)" : "var(--text)",
                textShadow: p.rank === 1 ? "0 0 10px rgba(246,207,125,0.5)" : isTop3 ? "0 0 8px rgba(114,240,255,0.4)" : "none",
              }}>
                {formatScore(p.score)}
              </div>
            </div>
